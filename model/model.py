...
from .configs import INPUT_DIM, MODEL_NAME, NUM_UNQ_CHARS
...
def res_block(ip, num_cnn_layers, cnn_filters, cnn_kernel_size, use_resnet):
    ...
    for _ in range(num_cnn_layers):
        res_arch = layers.Conv1D(cnn_filters, cnn_kernel_size, padding="same")(res_arch)
...
def get_model(
    ...
    for _ in range(num_res_blocks):
        arch = res_block(arch, num_cnn_layers, cnn_filters, cnn_kernel_size, use_resnet)
    ...
    for _ in range(num_rnn_layers):
        if use_birnn:
            arch = layers.Bidirectional(
                rnn(rnn_dim, dropout=rnn_dropout, return_sequences=True)
            )(arch)
        else:
            arch = rnn(rnn_dim, dropout=rnn_dropout, return_sequences=True)(arch)
